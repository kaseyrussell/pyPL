...
def center_piezos( positioners ):
    """ Center the piezos within their range. """
...
def get_joystick_position( joystick ):
    """ Returns joystick position in X,Y, and Z (where Z is throttle).
...
def toggle_fine_coarse_control():
    """ Toggle between fine and coarse control
...
def toggle_motor_piezo_control():
    """ Toggle between controlling the motors or the piezos. """
...
def update_position( positioners, xyz ):
    """ The joystick has been moved, so update the appropriate motor or piezo
    position accordingly and send notice of the event to the main pyPL thread."""
    global fine_control
    global motors_or_piezos
    global focusing
...
def handle_joystick_button_down( event, positioners, wobbler, autoit ):
    """ Deals with a joystick button press. """
    global fine_control
    global motors_or_piezos
...
def handle_joystick_button_up( event ):
    """ Deals with release of a joystick button (although there may not be anything
    meaningful that happens as a result of this. """
    if ( event.dict['button'] == focusing_button ):
        #print "focusing done."
        global focusing
        focusing = False
...
def monitor_joystick( app, positioners, joysticks ):
    """ Monitor the joystick for changes in position or button presses. """
...
def StartControl( app, positioners ):
    """ Initialize pygame and start the monitor_joystick event loop. """
